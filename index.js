...
class YTDownload {
    ...
    async #getAudio(url) {
        ...
        console.log(chalk.greenBright("Started:"), started.toLocaleTimeString());
        ...
        stream.on("progress", (e, downloaded, total) => {
            ...
            if (timeNow.getTime() - prevTime.getTime() < 200)
                return;
            spinner.update({
                text: `[${this.byteToMB(downloaded)} / ${this.byteToMB(total)} MB] @ ${this.byteToMB((downloaded - prevSize) /
                    ((timeNow.getTime() - prevTime.getTime()) / 1000))}MBps [${new Date(timeNow.getTime() - started.getTime())
                    .toISOString()
                    .substring(11, 19)}]`,
            });
            prevSize = downloaded;
    ...
    async #getVideo(url) {
        ...
        const update = () => {
            ...
            if (timeNow.getTime() - prevTime.getTime() < 200)
                return;
            spinner.update({
                text: `Audio: [${this.byteToMB(progress.audio)} / ${this.byteToMB(progress.audioTotal)} MB] @ ${this.byteToMB((progress.audio - progress.audioPrevSize) /
                    ((timeNow.getTime() - prevTime.getTime()) / 1000))}MBps [${new Date(progress.audioPreTime.getTime() - started.getTime())
                    .toISOString()
                    .substring(11, 19)}]\n` +
                    `  Video: [${this.byteToMB(progress.video)} / ${this.byteToMB(progress.videoTotal)} MB] @ ${this.byteToMB((progress.video - progress.videoPrevSize) /
                        ((timeNow.getTime() - prevTime.getTime()) / 1000))}MBps [${new Date(progress.videoPreTime.getTime() - started.getTime())
                        .toISOString()
                        .substring(11, 19)}]`,
            });
